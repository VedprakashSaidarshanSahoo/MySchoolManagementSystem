...
(async function() {
    ...
    const DATA_FOLDER = 'data_container';

    // Ensure the data folder exists in localStorage
    if (!localStorage.getItem(DATA_FOLDER)) {
        localStorage.setItem(DATA_FOLDER, JSON.stringify({}));
    }

    let timetableData = {};

    function saveTimetableDataToStorage(data) {
        localStorage.setItem('timetableData', JSON.stringify(data));
    }

    function getTimetableDataFromStorage() {
        return JSON.parse(localStorage.getItem('timetableData') || '{}');
    }
     ...
     * @param {object} classData - The class data to save
     */
    async function saveClassDataToFile(classData) {
        const folder = JSON.parse(localStorage.getItem(DATA_FOLDER));
        const fileName = `${classData.name}_${classData.section}`;
        folder[fileName] = classData;
        localStorage.setItem(DATA_FOLDER, JSON.stringify(folder));
    }
     ...
     * @returns {Promise<object|null>} - The class data
     */
    async function readClassDataFromFile(className, section) {
        const folder = JSON.parse(localStorage.getItem(DATA_FOLDER));
        const fileName = `${className}_${section}`;
        return folder[fileName] || null;
    }

    /**
     * Update class data in localStorage
     * @param {object} updatedClassData - The updated class data
     */
    async function updateClassDataInFile(updatedClassData) {
        const folder = JSON.parse(localStorage.getItem(DATA_FOLDER));
        const fileName = `${updatedClassData.name}_${updatedClassData.section}`;
        folder[fileName] = updatedClassData;
        localStorage.setItem(DATA_FOLDER, JSON.stringify(folder));
    }

    /**
     * Delete class data from localStorage
     * @param {string} className - The class name
     * @param {string} section - The section name
     */
    async function deleteClassDataFromFile(className, section) {
        const folder = JSON.parse(localStorage.getItem(DATA_FOLDER));
        const fileName = `${className}_${section}`;
    ...
    window.readClassDataFromFile = readClassDataFromFile;
    window.updateClassDataInFile = updateClassDataInFile;
    window.deleteClassDataFromFile = deleteClassDataFromFile;
})();
